...
def changeTheme():
    ...
    if themeVar=='dark':
        ...
        themeVar='light'
        listbox.config(bg='gray',fg='black')
        theme.configure(image=darkImage)
    elif themeVar=='light':
        customtkinter.set_appearance_mode('dark')
        themeVar='dark'
        theme.configure(image=brightImage)
        listbox.config(bg='#2a2c2e',fg='white')
...
theme=CTkButton(master,text='',font=("Georgia",15,"bold"),image=brightImage,hover=True,corner_radius=100,border_width=3,
          height=30,border_color="black",width=30,hover_color="blue",fg_color='navyBlue',command=changeTheme)
theme.place(x=590,y=10)
